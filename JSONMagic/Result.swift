...
/// A success `Result` if value is non nil failure with error otherwise
func optionalSuccess<T>(value: T?, error: NSError = NSError(domain: "", code: 0, userInfo: nil)) -> Result<T> {
    if let present:T = value {
        return .Success(Box(present))
    }
    else {
        return .Failure(error)
    }
